...
似たような発想の仕組みとして、「2枚の画像を見せて『どっちがエロいか』を選ばせる」ような実験が過去にもありました。  
今回のプロジェクトは「文章」で同じようなことをやろうとしていますが、**文章ならではのバイアス**が強く出やすいと考えています。

具体的には、次のような点です。

- 文章は上から順に読むので、どうしても **A（上）→B（下）** の順で目に入る
- A を読んでいる途中で「わかりやすい単語」「好みの表現」が出てくると、  
  その時点で「もう A でいいや」と判断してしまいやすい
- その結果、「内容の良し悪し」だけではなく、  
  **先に目に入る位置や、途中まで読んだ時点の印象**で選択が偏る可能性が高い

画像の場合もレイアウトのバイアスはありますが、  
文章は「読み進める」というプロセスがある分、これがより強く効くと考えています。

このプロジェクトでは、そうしたバイアスを少しでも減らす・測るために、例えば次のようなことを考えています。


完全にバイアスを消すことはできませんが、  
「少なくともこういう偏りがあるはずだ」「それを前提にどう補正するか」を最初から考えて、実装しています
